...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)//, float size)
{
    ...
    ImU32 col = !held ? ImColor(255,102,102,255) : ImColor(255,38,38,255);
    ImVec2 center = bb.GetCenter(); center.x += 0.05f * g.FontSize; // [Edited] : center.x += -3.0f
    if (hovered){
        //window->DrawList->AddCircleFilled(center, ImMax(2.0f, g.FontSize * 0.5f + 1.0f), col, 12);
        // [Edited] (rect exit button istead of Circle) : flo....ol);
        float rc_size = 0.58f * g.FontSize;
        window->DrawList->AddRectFilled(ImVec2(center.x - rc_size, center.y + rc_size), ImVec2(center.x + rc_size,center.y - rc_size), col);
    }
